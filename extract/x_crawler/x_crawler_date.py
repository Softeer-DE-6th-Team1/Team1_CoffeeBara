# lambda_function.py
import os, asyncio, logging, csv, httpx, json
from datetime import datetime, timezone
import boto3, botocore
from twikit import Client
from twikit.errors import TooManyRequests, BadRequest
from httpx import ReadTimeout, ConnectTimeout, TimeoutException, ConnectError


logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
logging.getLogger("httpx").setLevel(logging.WARNING)   # ← httpx INFO 로그 숨김

# ===== 환경변수 =====
USERNAME = os.getenv('USERNAME', '')
EMAIL = os.getenv('EMAIL', '')
PASSWORD = os.getenv('PASSWORD', '')

CHANNEL = os.getenv('CHANNEL', 'X')
KEYWORD = os.getenv('KEYWORD', "tesla autopilot")
safe_kw = KEYWORD.replace("/", "_").replace(" ", "-")
SINCE_DATE = os.getenv('SINCE_DATE', '2025-08-01')
UNTIL_DATE = os.getenv('UNTIL_DATE', '2025-08-03')
LANG_CODE = os.getenv('LANG_CODE', 'en')
TARGET = int(os.getenv("TARGET_ROWS", "200"))

S3_BUCKET = os.getenv('S3_BUCKET', 'demo-hayeonnoh')
S3_PREFIX = os.getenv('S3_KEY_PREFIX', 'twitter-data')
COOKIES_KEY = os.getenv("COOKIES_KEY", "secret/cookies.json")
STATE_KEY = f"{S3_PREFIX}/state/{safe_kw}.json"

s3 = boto3.client('s3')


def _s3_upload(local_path: str, key: str):
    if not S3_BUCKET:
        raise RuntimeError("S3_BUCKET env is empty")
    s3.upload_file(local_path, S3_BUCKET, key)
    logging.info(f"uploaded to s3://{S3_BUCKET}/{key}")


def _csv_header(path: str):
    if not os.path.exists(path):
        with open(path, "w", newline="", encoding="utf-8-sig") as f:
            w = csv.DictWriter(f, fieldnames=[
                "username","uploaded_time","collected_time","channel","text"
            ])
            w.writeheader()


def save_state(cursor, oldest_iso):
    state = {"cursor": cursor, "oldest_seen": oldest_iso}
    s3.put_object(Bucket=S3_BUCKET, Key=STATE_KEY, Body=json.dumps(state).encode())


def load_state():
    try:
        obj = s3.get_object(Bucket=S3_BUCKET, Key=STATE_KEY)
        return json.loads(obj["Body"].read())
    except botocore.exceptions.ClientError:
        return None


# UTC 경계 계산
since_dt = datetime.fromisoformat(SINCE_DATE).replace(tzinfo=timezone.utc)
until_dt = datetime.fromisoformat(UNTIL_DATE).replace(tzinfo=timezone.utc)

def in_window(created: datetime) -> bool:
    # since <= created < until
    return (created >= since_dt) and (created < until_dt)

# 중간 배치 로그 확인용
def _parse_created_at(raw):
    """twikit의 created_at을 UTC-aware datetime으로 변환"""
    if raw is None:
        return None
    if isinstance(raw, datetime):
        return raw if raw.tzinfo else raw.replace(tzinfo=timezone.utc)
    # 예: "Mon Aug 19 02:15:23 +0000 2025"
    try:
        dt = datetime.strptime(raw, "%a %b %d %H:%M:%S %z %Y")
        return dt.astimezone(timezone.utc)
    except Exception:
        return None

def _batch_time_range(batch):
    """배치에서 (최신, 가장오래된) UTC 시각 범위 튜플 반환"""
    times = []
    for t in batch:
        dt = _parse_created_at(getattr(t, "created_at", None))
        if dt:
            times.append(dt)
    if not times:
        return None, None
    newest = max(times)
    oldest = min(times)
    return newest, oldest

# 최신→과거 방향으로 내려가므로, since 이전이면 중단
def is_too_old(batch) -> bool:
    for t in batch:
        created = getattr(t, "created_at", None)
        if isinstance(created, str):
            try:
                created = datetime.strptime(created, "%a %b %d %H:%M:%S %z %Y")
            except Exception:
                continue
        if created and created.tzinfo is None:
            created = created.replace(tzinfo=timezone.utc)
        if created and created < since_dt:
            return True
    return False


def _save_batch(path: str, batch, collected_time_iso: str) -> int:
    saved = 0
    with open(path, "a", newline="", encoding="utf-8-sig") as f:
        w = csv.DictWriter(f, fieldnames=[
            "username","uploaded_time","collected_time","channel","text"
        ])
        for t in batch:
            text = (getattr(t, "text", "") or "").replace("\n", " ").strip()
            # 본문에 키워드가 있을 때만 저장
            # if KEYWORD.lower() not in text.lower():
                # continue

            # 1) created_at 파싱/타임존 보정
            created = getattr(t, "created_at", None)
            # twikit 버전에 따라 str일 수 있어 방어
            if isinstance(created, str):
                # 예: "Mon Aug 19 02:15:23 +0000 2025"
                try:
                    created = datetime.strptime(created, "%a %b %d %H:%M:%S %z %Y")
                except Exception as e:
                    logging.warning(f"created_at parse failed: {e}; raw={created!r}")
                    created = None
            if created is not None and created.tzinfo is None:
                created = created.replace(tzinfo=timezone.utc)
            uploaded_iso = created.astimezone(timezone.utc).isoformat(timespec="seconds") if created else None

            # 2) 창 밖은 스킵 (날짜 필터)
            if not created or not in_window(created.astimezone(timezone.utc)):
                continue

            # 3) 언어 필터 (tweet.lang 있으면 활용)
            tw_lang = getattr(t, "lang", None)
            if tw_lang and tw_lang != LANG_CODE:
                continue

            w.writerow({
                "username": getattr(getattr(t, "user", None), "name", ""),
                "uploaded_time": uploaded_iso,
                "collected_time": collected_time_iso,
                "channel": CHANNEL,
                "text": text,
            })
            saved += 1
    return saved


async def run(context=None):
    client = Client(
    'en-US',
    timeout=httpx.Timeout(connect=15, read=45, write=30, pool=30)
    )

    if not S3_BUCKET:
        raise RuntimeError("S3_BUCKET env is required")

    # ---- 남은 시간 체크 유틸 ----
    def time_ms_left():
        try:
            return context.get_remaining_time_in_millis() if context else 300000
        except Exception:
            return 300000

    def has_time_left(buffer_ms=15000):
        return time_ms_left() > buffer_ms


    # ---- 쿠키 파일 준비 (/tmp) ----
    # cookies는 람다에서 /tmp에 보관
    cookies_path = "/tmp/cookies.json"
    try:
        s3.download_file(S3_BUCKET, COOKIES_KEY, cookies_path)
        logging.info("cookies.json downloaded from S3")
    except botocore.exceptions.ClientError as e:
        logging.error(f"cookies download failed: {e}")
        raise

    # ---- 로그인 ----
    try:
        # 쿠키 로그인 (동기)
        await client.login( 
            auth_info_1=USERNAME, 
            auth_info_2=EMAIL, 
            password=PASSWORD, 
            cookies_file=cookies_path)
        logging.info("cookie load OK")

    except Exception as e:
        logging.error(f"[FATAL] cookie login failed: {type(e).__name__}: {e}")
        raise


    collected_iso = datetime.now(timezone.utc).isoformat(timespec="seconds")
    local_csv = f"/tmp/x_outputs_{int(datetime.now().timestamp())}.csv"
    _csv_header(local_csv)

    query = KEYWORD
    logging.info(f"search start: '{query}'")

    # ---- 첫 페이지 ----
    state = load_state()
    cursor = state.get("cursor") if state else None
    pages = 0
    total_saved = 0

    if cursor is None:
        try:
            page = await client.search_tweet(query, "Latest", count=20)
        except Exception as e:
            logging.error(f"[FATAL] initial search failed: {e}")
            raise
        
        saved = _save_batch(local_csv, page, collected_iso)
        total_saved += saved
        pages += 1

        newest, oldest = _batch_time_range(page)
        logging.info(
            f"[page {pages}] saved={saved} total_saved={total_saved} "
            f"range=({oldest and oldest.isoformat(timespec='seconds')} ~ {newest and newest.isoformat(timespec='seconds')})"
        )
        if oldest:
            save_state(cursor, oldest.isoformat(timespec="seconds"))
        cursor = getattr(page, "next_cursor", None)

    # ---- 페이지네이션 ----
    while cursor and has_time_left(20000): # and fetched < TARGET
        # 여유 시간 부족하면 부분 업로드 후 종료
        if not has_time_left(20000):
            logging.warning("Low time budget detected, stopping pagination")
            break

        await asyncio.sleep(3)

        try:
            page = await client.search_tweet(query, "Latest", count=20, cursor=cursor)
        except TooManyRequests:
            # 남은시간 부족하면 중단 후 부분 업로드
            logging.warning("429: backing off 10s"); await asyncio.sleep(10)
            try:
                page = await client.search_tweet(query, "Latest", count=20, cursor=cursor)
            except TooManyRequests:
                logging.warning("429 again; save checkpoint & stop")
                save_state(cursor, oldest and oldest.isoformat(timespec="seconds"))
                break
        except (TimeoutException, ConnectError, ConnectTimeout) as e:
            logging.warning(f"search page transient error: {e}; stop paging")
            break

        if not page:
            break

        saved = _save_batch(local_csv, page, collected_iso)
        total_saved += saved
        pages += 1

        newest, oldest = _batch_time_range(page)
        logging.info(
            f"[page {pages}] saved={saved} total_saved={total_saved} "
            f"range=({oldest and oldest.isoformat(timespec='seconds')} ~ {newest and newest.isoformat(timespec='seconds')})"
        )

        # 배치 중 가장 오래된 트윗이 since 이전이면 더 내려갈 필요 없음
        if is_too_old(page):
            logging.info("old enough; stop pagination")
            break
        cursor = getattr(page, "next_cursor", None)

    # ---- 업로드 & 정리 ----
    ts = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    key = f"{S3_PREFIX}/keyword={safe_kw}/{ts}.csv"
    
    try:
        _s3_upload(local_csv, key)
    finally:
        # 파일 정리
        try:
            os.remove(local_csv)
        except Exception:
            pass

    # (선택) httpx 커넥션 정리
    try:
        await client.http.aclose()
    except Exception:
        pass

    result = {"ok": True, "rows_saved": total_saved, "s3_key": key}
    if not has_time_left(0):
        # (로그 목적) 거의 소진 상태
        result["near_timeout"] = True
    return result

def lambda_handler(event, context):
    """
    AWS Lambda entrypoint.
    context.get_remaining_time_in_millis()를 run()에 전달하여
    429/지연 시 부분 업로드 후 안전 종료하도록 함.
    """
    return asyncio.run(run(context))
